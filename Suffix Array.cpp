...
void suffix_array(int *s, int *SA, int n, int K) {
    ...
    for (int p = 0, t = n0 - n1, k = 0; k < n; k++) {
        ...
        int i = GetI(), j = SA0[p];
        if (SA12[t] < n0 ?  leq<2>({s[i], s12[SA12[t] + n0]}, {s[j], s12[j / 3]}) : 
                            leq<3>({s[i], s[i + 1], s12[SA12[t] - n0 + 1]}, {s[j], s[j + 1], s12[j / 3 + n0]})) {
            SA[k] = i;
            if (++t == n02) {
                for (k++; p < n0; p++, k++) {
                    SA[k] = SA0[p];
...
long long countDistinctSubstrings(const std::vector<int> &lcp) {
