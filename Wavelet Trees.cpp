/*  
 *  https://codeforces.com/blog/entry/89110?#comment-775550
 *  Wavelet tree with low memory usage.
 *  Supports several orthogonal queries in O((log n)^2)
 *
 *  Also supports activate / deactivate updates, which can
 *  be used for other types of updates in an offline setting.
 *
...
public:
    ...
    Wavelet_Matrix(vector<T> v): n(v.size()), data(height, n), active(height, n){
        build(move(v));
    }
...
int main() {
    ...
    Wavelet_Matrix<int> wt(A);
    while (q--) {
        int l, r, k;
        std::cin >> l >> r >> k;
        std::cout << wt.k_th(l, r, k) << '\n';
    }
    return 0;
}